...
Although we may think that most sensors are single-input single output (SISO) like temperature sensors, there are sensors with single input and multiple outputs such as gas sensors \cite{Munther19}, and sensors with multiple inputs and multiple outputs like the three-axis accelerometers \cite{DEmilia16}, the radio-frequency intruder-detection sensors \cite{Ushiki13}, and the radar sensor \cite{Kueppers17}.
...
\begin{equation} \begin{aligned} \mathbf{x}_\text{a}(k+1) &= \underbrace{ \begin{bmatrix} \mathbf{A} & \mathbf{B} \\ 0 & 1 \end{bmatrix} }_{\mathbf{A}_\text{a}} \mathbf{x}_\text{a}(k) , \quad \mathbf{y}(k) = \underbrace{ \begin{bmatrix} \mathbf{C} & \mathbf{D} \end{bmatrix} }_{\mathbf{C}_\text{a}} \mathbf{x}_\text{a}(k), \quad \text{where} \ \ \mathbf{x}_\text{a}(k) = \begin{bmatrix} \mathbf{x}(k) \\ \mathbf{u}(k) \end{bmatrix}, \ \ \mathbf{x}_{\text{ini}} = \mathbf{x}(0) . \label{eqn:augmented} \end{aligned} \end{equation}
...
Thus, we can express 
\[ \mathbfcal{H}(\mathbf{y}) = \begin{bmatrix} y(1) & y(2) & \cdots & y(n) \\ y(2) & y(3) & \cdots & y(n+1) \\ \iddots & \iddots & \iddots \\ y(n) & y(n+1) & \cdots & y(2n-1) \end{bmatrix} = \begin{bmatrix} \widehat{\mathbf{C}}_\text{a} \\ \widehat{\mathbf{C}}_\text{a} \widehat{\mathbf{A}}_\text{a} \\ \vdots \\ \widehat{\mathbf{C}}_\text{a} \widehat{\mathbf{A}}_\text{a}^{n} \end{bmatrix}  \begin{bmatrix} \mathbf{x}_\text{a}(0) & \mathbf{x}_\text{a}(1) & \cdots & \mathbf{x}_\text{a}(n) \end{bmatrix} = \mathbfcal{O}_\text{a} \mathbf{X}_\text{ini}, \]
...
The step response observations ${y}(t) \in \mathbb{R}^{}$ for $t = 1,\ldots,T$, where $T$ is the number of samples, is perturbed by normally distributed measurement noise $\epsilon$ with zero mean and given variance $\sigma_{\epsilon}^2$.
 ...
 The step input estimation method was described in \cite{Markovsky15cep}.
...
is a Hankel matrix of $(T - n)-\text{block rows}$, constructed from consecutive differences $\Delta \widetilde{y}(t) = \widetilde{y}(t) - \widetilde{y}(t-1)$ of the observed transient response, augmented in the left side with a $(T - n)\text{-vector}$ of elements equal to the known dc-gain $g$, (see \cite{Markovsky15cep}).
...
Details of the step input estimation method are described in \cite{Markovsky15cep}.
...
The estimation of the step input level is obtained as the solution of the minimization problem \cite{Markovsky15ieee, Markovsky15cep} 
\begin{equation} \widehat{\mathbf{x}} = \underset{\mathbf{x}}{\mathrm{argmin}} \ \left\Vert  \widetilde{\mathbf{y}} - \widetilde{\mathbf{K}} \mathbf{x} \right\Vert^2_2 \label{eqn:min_ls} \end{equation}
